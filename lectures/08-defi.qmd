...
| | |
|---|---|
| **pool price** | `y / x` — where the pool sits right now |
| **execution price** | what you actually paid, on average, across the trade |
| **pool price after** | where it sits once you are done |

. . .

The difference between the first and the second is **price impact**, and it grows with your size relative to the pool. A fee is charged on top of all three.

[A big order in a small pool pays badly, and no amount of shopping around changes it: the curve is the market.]{.red}
...
$$\frac{\text{LP value}}{\text{hold value}} - 1 = \frac{2\sqrt{r}}{1+r} - 1 \;\leq\; 0$$

. . .

**Always negative** when the price moves, in either direction.

The benchmark is **holding the two assets**, never your initial deposit. An LP can be up in euros and still have lost to doing nothing — which is the comparison that decides whether the position made sense.

[Fee income is the payment for bearing this. Fee yield ≈ fee × (daily volume ÷ TVL) × 365. Compare the two, or you are not analysing anything.]{.red}
