...
| | | |
|---|---|---|
| 1–2 | 23–30 Sep | the model, the data, the two silent errors ✓ |
...
The split is the obvious way future information gets in. The dangerous way is through a **feature**.

| feature | what is wrong with it |
|---|---|
| a rolling mean computed over the whole series | every row knows the future |
| a fundamental dated by the quarter it *describes* | you could not read it until weeks later |
| a normalisation using the full-sample standard deviation | the scale itself contains tomorrow |

::: {.callout-tip title="The only defence"}
For every feature, write the **latest row it touches**. If that row is later than `t`, it is a leak — and no validation scheme will find it for you.
:::
...
A prediction becomes a decision only when you add two things it does not contain:

1. **What each mistake costs.** A missed fraud is not the same size as a blocked customer.
2. **A threshold.** The probability above which you act — chosen from those costs, **never** left at 0.5.

. . .

$$\text{expected value} = p \cdot (\text{gain}) - (1-p) \cdot (\text{cost})$$

[Act when that is positive. The threshold falls straight out of it, and it is almost never a half.]{.red}
