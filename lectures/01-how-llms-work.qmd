...
| **11:40** | a real model release, taken apart |
| **11:50** | close |
...
| `egg` · `Egg` · ` egg` · `EGG` | four different tokens |
...
## Now, three sentences from three pitch decks

::: {.callout-important title="Which are impossible? Which is unanswerable?"}
...
## One: the model remembers nothing {.statement}

[Something else stores the history and re-sends it, every turn.]{.muted}

## Two: the weights are frozen when you call it {.statement}

[A correction goes into a database, or into a retraining queue. Ask which.]{.muted}

## Three: accurate compared with what? {.statement}

[That needs a base rate. Week 3.]{.muted}
