...
| **10:25** | what this course is for, and how we shape it around you |
| **10:43** | open your notebook — one click |
| **10:50** | break |
| **11:00** | from one neuron to an LLM |
| **11:35** | what the model actually reads: tokens, attention, the box |
| **11:46** | between Wednesdays, and homework |
...
:::: {.columns}
::: {.column width="62%"}
[LAB · week 1 notebook]{.kicker}

1. **Click the link, or scan the code.** Colab opens the notebook.
2. `File ▸ Save a copy in Drive` — now it is yours.
...
[GitHub comes at home, with the [setup page](../setup.qmd) — ten minutes.]{style="font-size:.7em"}
:::
::: {.column width="38%"}
...
You have heard a version of all three in your work. **Two are impossible as stated. One is unanswerable as written.** Write down which is which — before you leave you can answer two of them yourself.
...
## The squash has a name: the **activation function**

![](img/activations.svg){fig-align="center" width="86%"}

[**Weights**: how much each input matters. **Bias**: how trigger-happy the neuron is. **Activation**: what it passes on.]{.muted}

## Training it by hand: four rows, one rule {.clip background-color="#000000"}
...
Wrong answer → move each weight a little towards the right one. Right answer → leave it alone. Repeat until nothing changes. **That is learning.**
...
[Think of the parameters as the knobs on a DJ mixer. Training is finding a setting of billions of knobs that makes the output match the data. — Karpathy]{.muted}
...
About 15 trillion tokens of filtered web text. Months of GPUs, done **once a year or so**. Out comes a **base model** — an *internet document simulator*. Ask it a question and it may answer with three more questions.
:::
::: {.column width="33%"}
**2 · Post-training**

Swap the data: on the order of 100,000 example conversations, written or checked by people, then reinforcement learning on what gets rated well. Days, not months — **every week or so**. Out comes an **assistant**.
:::
::: {.column width="33%"}
**3 · What you download**

Two files: the **parameters** — billions of numbers — and ~500 lines of code that run them. That is a model.
:::
::::
...
[Lossy, and oddly: it knows Tom Cruise's mother is Mary Lee Pfeiffer, but not whose son Mary Lee Pfeiffer's is. You evaluate these models; you cannot read them.]{.red}

## Training an LLM is the same nudge, a trillion times

Take four tokens from the training text — `The ECB left rates` — and ask for the next one. The right answer is known: it is in the text.

| next token | before the update | after |
|---|---|---|
| **"unchanged"** — the right one | 3% | **4%** |
| "steady" | 4% | 3.8% |
| "at" | 2% | 1.9% |
| …the other 200,000 | the rest | slightly less |

. . .

[Nudge the knobs so the right token gets slightly more likely. Do it for every token of 15 trillion, many times over. What comes out is a model of **how text continues** — and nothing else. It has **no memory**: a fixed expression from input to output.]{.red}
...
The network reads a sequence, and **shorter sequences are cheaper**. So the text is compressed before it goes in:

| the same 5,000 characters, as | length | alphabet |
|---|---|---|
| bits | 40,000 | 2 symbols |
| bytes | 5,000 | 256 symbols |
| GPT-4 tokens | **~1,300** | **100,277 symbols** |

[A tokenizer is learned by merging the pairs that occur most often, again and again. It learns the language of its training data — which is why *your* language costs more. (Karpathy's example.)]{.muted}
...
- **Andrej Karpathy, *Intro to Large Language Models*** (YouTube, 2023) — one hour, and the source of the "two files" and "compressing the internet" pictures.
...
[Next week: we read a real model release line by line, the assistant writes the code and you decide whether it is true — and you train your first tokenizer.]{.muted}

## [Spare]{.section-kicker} For next week, or if there is time {.section-slide background-color="#7a7f85"}
