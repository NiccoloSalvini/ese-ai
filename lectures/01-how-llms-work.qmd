...
| payment | large? (over €1,000) | abroad? | fraud? |
|---|---|---|---|
| **A** | 0 | 0 | no |
| **B** | 1 | 0 | no |
| **C** | 0 | 1 | no |
...
A first guess: weight **1** on "large", **0** on "abroad", bias **−1**; block if the score is above zero. Which payments does it block?
:::

[From here to the end of the lesson, one question: **is this payment fraud?** Every step makes the answer a little more capable.]{.explain}
...
$$w \;\leftarrow\; w + \eta \cdot (\text{fraud} - \text{decision}) \cdot x$$

| | w large | w abroad | bias |
|---|---|---|---|
| first guess | 1 | 0 | −1 |
| after 2 mistakes | **1** | **0.5** | **−1** |

[Nobody wrote "large **and** abroad". The weights found it from four examples. This is Software 2.0 — and this neuron has a name: the **perceptron**, 1958.]{.explain}
...
The **loss**: small when the model gave the right answer a high probability.

| payment | p(fraud) | right answer | loss = −log p(right answer) |
|---|---|---|---|
| **D** | 0.62 | fraud | −log 0.62 = **0.47** |
| **B** | 0.50 | not fraud | −log 0.50 = **0.69** |

[Training means one thing: change the weights so this number goes down.]{.explain}

## The formula for every weight

$$w \;\leftarrow\; w - \eta \cdot (p - y) \cdot x$$

One step on payment D, with η = 1:

| | w large | w abroad | bias | p(fraud) for D |
|---|---|---|---|---|
| before | 1 | 0.5 | −1 | 0.62 |
| blame = (p − y)·x | −0.38 | −0.38 | −0.38 | |
| after | **1.38** | **0.88** | **−0.62** | **0.84** |

[Same shape as the perceptron rule — with a probability in place of a yes/no. D is now 84% fraud instead of 62%.]{.explain}
...
[Add payment E — a customer on holiday, large, abroad, genuine — and there is no perfect rule. Too small a step crawls; too large jumps over the best answer.]{.explain}
...
[Exactly the step we took on payment D: raise the probability of the right answer, lower the others. Here the right answer is in the text itself.]{.explain}
...
| **117B total parameters** | our neuron had 3 weights; this has 117 billion |
