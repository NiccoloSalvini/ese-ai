...
| | tokens | vs English |
|---|---|---|
| English sentence | 12 | 1.00× |
| Italian, same meaning | 17 | 1.42× |
| Russian, **GPT-4 tokenizer** | 33 | 2.75× |
| Russian, **GPT-4o tokenizer** | 18 | 1.50× |

. . .

[A Russian-language pilot priced on the first tokenizer and delivered on the second lost **45% of its unit cost** to a change nobody in the room chose. This is not a detail. It is the sort of thing that decides whether a pilot clears its business case.]{.red}
...
The attention step compares every token with every other token. **Double the context and that step costs roughly four times as much**, not twice.

. . .

Worse, it is not uniform. Put a fact at the start, at the middle and at the end of a very long input and ask for it back: **the middle is recovered least often**. Measured repeatedly, across model families.
...
| what is actually done | changes | costs | shows up in |
|---|---|---|---|
| **put it in the prompt** | nothing, ever | per call, for ever | seconds |
| **retrieval** | what it can look up | an index, plus per call | days |
| **fine-tuning** | the model's *style and format* | a training run, repeated | weeks |

. . .

[Fine-tuning teaches a model **how to say things**. It is an unreliable way to teach it **what is true** — a fact you fine-tuned in cannot be updated, cited, or withdrawn. When a vendor says "we train on your data", the question is which of these three, and you will not get a straight answer unless you name them.]{.red}
